...
client.on('ready', () => {
  ...
  client.guilds.forEach(function (value, key) {
    if (con.query(`SELECT guildID FROM Servers WHERE EXISTS (${key} = Servers.guildID);`), (err, res) => {
        console.log(res); 
        console.log(err);
      }
    ) {}
    else {
      con.query(`INSERT INTO Servers (guildName, guildID) VALUES (\'${value.toString()}\', ${key.valueOf()});`), (err, res) => {
        console.log(res);
        console.log(err);
      };
    }
